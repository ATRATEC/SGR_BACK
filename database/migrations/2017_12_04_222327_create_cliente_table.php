...
class CreateClienteTable extends Migration {
	...
	public function up()
	{
		Schema::create('cliente', function(Blueprint $table)
		{
			...
			$table->bigInteger('codigo_omie')->index('idx_cliente')->comment('Código do cliente - gerado pelo Omie.');
			$table->string('codigo_integracao', 20)->nullable()->index('idx2_cliente')->comment('Código de Integração com sistemas legados.');
			$table->string('cnpj_cpf', 20)->nullable()->comment('CNPJ / CPF do cliente.');
			$table->string('razao_social', 60)->nullable()->comment('Razão Social do cliente');
			$table->string('nome_fantasia', 50)->nullable()->comment('Nome Fantasia do cliente');
